...
class MockSlackClient:
    ...
    def api_call(self, api_function, text=None, channel=None, ts=None, username=None, as_user=None, user=None):
        ...
        if user:
            return json.dumps({'user': {'name': user}})
        if "im.list" == api_function:
            return json.dumps({'ims': [{'user': 'p1', 'id':'p1_dm'}, {'user': 'p2', 'id':'p2_dm'}, {'user': 'p3', 'id':'p3_dm'}, {'user': 'p4', 'id':'p4_dm'}, {'user': 'p5', 'id':'p5_dm'}]})
        else:
            self.ts += 1
            return json.dumps({'ts':self.ts})